...
* system.
*/
@JSExportAll  case class IliadOrthographyValidator(lib: CiteLibrary) extends CiteValidator[IliadString] with LogSupport  {
  ...
  def label: String = "Validate orthography of IliadStrings"

  // 2. required by CiteValidator trait
  def validate(library: CiteLibrary) : Vector[TestResult[IliadString]] = {
  ...
  def validate(surface: Cite2Urn) : Vector[TestResult[IliadString]] =  {
  ...
  def verify(surface: Cite2Urn) : String = {
    ...
    val hdr = s"# Verification of `IliadString` orthography\n\nSurface: ${surface}\n\n"
  ...
  def tokensForUrn(psg: CtsUrn) : Vector[Vector[MidToken]]= {
    ...
    for (n <- subcorpus.nodes) yield {
      IliadOrthography.tokenizeNode(n)
    }
  }
  ...
  def validate (c: Corpus): Vector[TestResult[IliadString]]  = {
  ...
  * expected to follow IliadString.
  */
  def validate(textNode: CitableNode) : Vector[TestResult[IliadString]] = {
    val tokens = IliadOrthography.tokenizeNode(textNode)
    tokens.isEmpty match {
      case true => {
        warn("No tokens found from " + textNode)
        Vector.empty[TestResult[IliadString]]
      }

      case false => {
        val lgsList = tokens.map(t => (t.urn, IliadString(t.text)))

        for( (urn, lgs) <- lgsList) yield {
          IliadOrthography.validString(lgs.ascii) match {
            case true => TestResult(true, s"${lgs.ucode} (${urn}) valid.", lgs)
            case false => TestResult(false, s"${lgs.ucode} invalid (${urn}):  ${IliadOrthography.hiliteBadCps(lgs.ascii)} ", lgs)
          }
        }
      }
    }
