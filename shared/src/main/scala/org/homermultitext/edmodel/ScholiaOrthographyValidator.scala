...
* system.
*/
@JSExportAll  case class ScholiaOrthographyValidator(lib: CiteLibrary) extends CiteValidator[ScholiaString] with LogSupport  {
  ...
  def label: String = "Validate orthography of ScholiaStrings"

  // 2. required by CiteValidator trait
  def validate(library: CiteLibrary) : Vector[TestResult[ScholiaString]] = {
  ...
  def validate(surface: Cite2Urn) : Vector[TestResult[ScholiaString]] =  {
  ...
  def verify(surface: Cite2Urn) : String = {
    ...
    val hdr = s"# Verification of `ScholiaString` orthography\n\nSurface: ${surface}\n\n"
  ...
  def validate (c: Corpus): Vector[TestResult[ScholiaString]]  = {
  ...
  * expected to follow ScholiaString.
  */
  def validate(textNode: CitableNode) : Vector[TestResult[ScholiaString]] = {
    val tokens = ScholiaOrthography.tokenizeNode(textNode)
    tokens.isEmpty match {
      case true => {
        warn("No tokens found from " + textNode)
        Vector.empty[TestResult[ScholiaString]]
      }

      case false => {
        val lgsList = tokens.map(t => (t.urn, ScholiaString(t.text)))

        for( (urn, lgs) <- lgsList) yield {
          ScholiaOrthography.validString(lgs.ascii) match {
            case true => TestResult(true, s"${lgs.ucode} (${urn}) valid.", lgs)
            case false => TestResult(false, s"${lgs.ucode} invalid (${urn}):  ${ScholiaOrthography.hiliteBadCps(lgs.ascii)} ", lgs)
          }
        }
      }
    }
