...
*  ==Example==
*  The [[TeiReaderOld]] reads data in the OHCO2
...
* Example:
*  {{{
*  val tokenPairs = TeiReaderOld.fromCorpus(CORPUS_OBJECT)
*  }}}
*
*  ==How it works==
*  The [[TeiReaderOld]] object maintains three mutable buffers,
...
/// refactor this
case class TeiReaderOld(twoColumns: String, delimiter: String = "#") extends MidMarkupReader {

  def editionType: MidEditionType = MidDiplomaticEdition


  def editionTypes: Vector[MidEditionType] =  TeiReaderOld.editionTypes
  ...
  * to the TeiReaderOld's `wrappedWordBuffer`
  *
  ...
  *
  * Results are added to the TeiReaderOld's `tokenBuffer`.
  *
...
//hmtToken,
  ...
  */
  ...
  def tokens :Vector[TokenAnalysis] = {
    ...
    //pairArray.flatMap{ case (u,x) => TeiReaderOld.teiToTokens(u,x) }.toVector
  ...
  */
  def tokensFromNodeVector(nodes: Vector[CitableNode], tokens: Vector[TokenAnalysis]): Vector[TokenAnalysis] = {
    val n = nodes.head
    //val newTokens = tokens ++ TeiReaderOld.teiToTokens(n.urn, n.text, tokens.size)
