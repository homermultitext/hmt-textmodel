...
"""
  ...
  it should "use n attribute of TEI persName element for lexical disambiguation" in  {
    ...
    val xml = """<div xmlns="http://www.tei-c.org/ns/1.0" n="comment"> <p> καὶ <q> πόρε <persName n="urn:cite2:hmt:pers.v1:pers115"> Xείρων</persName></q> ⁑</p></div>"""

    val tokenV = TeiReader.teiToTokens(urn, xml)
    val pn = tokenV(2).analysis
    assert(pn.lexicalDisambiguation == Cite2Urn("urn:cite2:hmt:pers.v1:pers115"))
  }
  ...
  it should "use  n attribute of TEI placeName element for lexical disambiguation" in {
    ...
    val xml = """<div xmlns="http://www.tei-c.org/ns/1.0" n="comment"> <p> <choice> <abbr> οτ</abbr> <expan> ὅτι</expan></choice> θηλυκῶς τὴν <placeName n="urn:cite2:hmt:place.v1:place6"> Ἴλιον</placeName></p></div>"""
    val tokenV = TeiReader.teiToTokens(urn, xml)
    val troy = tokenV(3).analysis
    assert(troy.lexicalDisambiguation == Cite2Urn("urn:cite2:hmt:place.v1:place6"))
  }
  ...
  it should "use n attribute of TEI rs element for lexical disambiguation type attribute is ethnic" in {
    ...
    val xml = """<div xmlns="http://www.tei-c.org/ns/1.0" n="comment"> <p> τῶν τοις <rs n="urn:cite2:hmt:place.v1:place6" type="ethnic"> Τρωσὶ</rs> συμμαχούντων ⁑</p></div>"""
    val tokenV = TeiReader.teiToTokens(urn, xml)
    val trojans = tokenV(2).analysis
    assert(trojans.lexicalDisambiguation == Cite2Urn("urn:cite2:hmt:place.v1:place6"))
  }
